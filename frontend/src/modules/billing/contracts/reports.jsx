            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white rounded-3xl border border-slate-200 p-5">
                  ...
                  <p className="text-2xl font-bold text-slate-900 mt-1 whitespace-nowrap">{formatDisplayCurrency(totalValue, defaultCurrency)}</p>
                </div>
                <div className="bg-white rounded-3xl border border-slate-200 p-5">
              ...
              {monthlyChartData.length > 0 && (
                <div className="bg-white rounded-3xl border border-slate-200 p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-semibold text-slate-900">Monthly Contracts Started</h3>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <AreaChart data={monthlyChartData}>
                      <defs>
                        <linearGradient id="colorValue" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--color-accent-invoicing)" stopOpacity={0.3} /><stop offset="95%" stopColor="var(--color-accent-invoicing)" stopOpacity={0} /></linearGradient>
                      </defs>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                      ...
                      <Tooltip />
                      <Area yAxisId="left" type="monotone" dataKey="value" stroke="var(--color-accent-invoicing)" fill="url(#colorValue)" strokeWidth={2} name="Value" />
          ...
          {loading ? <Spinner /> : error ? <ErrorState message={error} onRetry={fetchContracts} /> : fContracts.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {statusData.map((s) => (
                  <div key={s.name} className="bg-white rounded-3xl border border-slate-200 p-4 text-center">
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-6">
                <h3 className="text-sm font-semibold text-slate-900 mb-4">All Contracts by Status</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100">
                        <th scope="col" className="text-left py-3 px-3 font-medium text-slate-500 text-xs uppercase tracking-wider">Contract</th>
                        ...
                        <th scope="col" className="text-right py-3 px-3 font-medium text-slate-500 text-xs uppercase tracking-wider">Value</th>
                        <th scope="col" className="text-left py-3 px-3 font-medium text-slate-500 text-xs uppercase tracking-wider">Period</th>
                      </tr>
                    </thead>
                    <tbody>
                      {fContracts.slice(0, 20).map((c) => (
                        <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50">
                          <td className="py-3 px-3 font-medium text-slate-900">{c.contract_number || `#${c.id}`}</td>
                          <td className="py-3 px-3 text-slate-600">{c.customer_name || `#${c.customer_id}`}</td>
                          <td className="py-3 px-3">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
                              c.status === "active" ? "bg-emerald-100 text-emerald-700" :
                              c.status === "pending" || c.status === "draft" ? "bg-amber-100 text-amber-700" :
                              c.status === "expired" ? "bg-slate-100 text-slate-600" : "bg-red-100 text-red-700"
                            }`}>{c.status}</span>
                          </td>
                          <td className="py-3 px-3 text-right font-medium text-slate-900">{formatDisplayCurrency(c.value || c.total_value, defaultCurrency)}</td>
          ...
          {loadingExpiring ? <Spinner /> : (
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white rounded-3xl border border-slate-200 p-5">
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Expiring (90d)</p>
                  <p className="text-2xl font-bold text-amber-600 mt-1">{expiring.length}</p>
                </div>
                <div className="bg-white rounded-3xl border border-slate-200 p-5">
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Expiring Value</p>
                  <p className="text-2xl font-bold text-slate-900 mt-1 whitespace-nowrap">{formatDisplayCurrency(expiring.reduce((s, c) => s + parseFloat(c.value || c.total_value || 0), 0), defaultCurrency)}</p>
                </div>
                <div className="bg-white rounded-3xl border border-slate-200 p-5">
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Auto-Renewal</p>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-semibold text-slate-900">Expiring Contracts (Next 90 Days)</h3>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-100">
                          ...
                          <th scope="col" className="text-left py-3 px-3 font-medium text-slate-500 text-xs uppercase tracking-wider">{singular}</th>
                          <th scope="col" className="text-right py-3 px-3 font-medium text-slate-500 text-xs uppercase tracking-wider">Value</th>
                          <th scope="col" className="text-left py-3 px-3 font-medium text-slate-500 text-xs uppercase tracking-wider">End Date</th>
                          <th scope="col" className="text-left py-3 px-3 font-medium text-slate-500 text-xs uppercase tracking-wider">Auto-Renew</th>
                        </tr>
                      </thead>
                      <tbody>
                        {expiring.map((c) => (
                          <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50">
                            ...
                            <td className="py-3 px-3 text-slate-600">{c.customer_name || `#${c.customer_id}`}</td>
                            <td className="py-3 px-3 text-right font-medium text-slate-900">{formatDisplayCurrency(c.value || c.total_value, defaultCurrency)}</td>
                            <td className="py-3 px-3 text-slate-500 whitespace-nowrap">{c.end_date ? new Date(c.end_date).toLocaleDateString() : "—"}</td>
                            <td className="py-3 px-3">
                              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${c.auto_renew ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
                                {c.auto_renew ? "Yes" : "No"}
                              </span>
                            </td>
                          </tr>
          ...
          {loading ? <Spinner /> : error ? <ErrorState message={error} onRetry={fetchContracts} /> : monthlyChartData.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-3xl border border-slate-200 p-5">
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Avg Monthly Value</p>
                  <p className="text-2xl font-bold text-slate-900 mt-1 whitespace-nowrap">{formatDisplayCurrency(totalValue / Math.max(monthlyChartData.length, 1), defaultCurrency)}</p>
                </div>
                <div className="bg-white rounded-3xl border border-slate-200 p-5">
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Avg Contracts/Month</p>
                  <p className="text-2xl font-bold text-slate-900 mt-1">{(fContracts.length / Math.max(monthlyChartData.length, 1)).toFixed(1)}</p>
                </div>
              </div>
              <div className="bg-white rounded-3xl border border-slate-200 p-6">
                <h3 className="text-sm font-semibold text-slate-900 mb-4">Monthly Contract Value</h3>
                <ResponsiveContainer width="100%" height={300}>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-6">
                <h3 className="text-sm font-semibold text-slate-900 mb-4">Monthly Contract Count</h3>
                <ResponsiveContainer width="100%" height={300}>
