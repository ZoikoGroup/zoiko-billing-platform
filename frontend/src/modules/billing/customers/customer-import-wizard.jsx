...
import {
  ...
  RotateCcw, Copy, Eye, ChevronDown, Users,
} from "lucide-react";
...
const CUSTOMER_FIELDS = [
  ...
  { value: "alternate_email",   label: "Alternate Email",       required: false },
  ...
  { value: "customer_type",     label: "Customer Type",         required: false },
  { value: "status",            label: "Status",                required: false },
  { value: "billing_address",   label: "Billing Address",       required: false },
  { value: "shipping_address",  label: "Shipping Address",      required: false },
  { value: "billing_country",   label: "Billing Country",       required: false },
  { value: "shipping_country",  label: "Shipping Country",      required: false },
  { value: "payment_terms",     label: "Payment Terms",         required: false },
  { value: "currency",          label: "Currency",              required: false },
  { value: "credit_limit",      label: "Credit Limit",          required: false },
  { value: "credit_days",       label: "Credit Days",           required: false },
  { value: "price_list",        label: "Price List",            required: false },
  { value: "gst_number",        label: "GST Number",            required: false },
  { value: "vat_number",        label: "VAT Number",            required: false },
  { value: "pan",               label: "PAN",                   required: false },
  { value: "tin",               label: "TIN",                   required: false },
  { value: "tax_category",      label: "Tax Category",          required: false },
  { value: "tax_id",            label: "Tax ID",                required: false },
  { value: "tax_id_type",       label: "Tax ID Type",           required: false },
  { value: "notes",             label: "Notes",                 required: false },
  { value: "tags",              label: "Tags",                  required: false },
  { value: "__ignore",          label: "— Ignore this column —", required: false },
];

// Mirrors the backend's MAX_IMPORT_FILE_SIZE_BYTES (customer_import_service.py)
const MAX_IMPORT_FILE_SIZE_BYTES = 10 * 1024 * 1024;
// Rows processed per confirm request — keeps each request fast and gives a real
// progress bar between batches instead of one long blocking call.
...
const DUPLICATE_STRATEGIES = [
  ...
  { value: "create_copy", label: "Create Copy",         icon: Copy,         desc: "Add as a new record with modified code" },
...
// ─── Main component ───────────────────────────────────────────────────────────
export default function CustomerImportWizard({ onClose, onImported }) {
  ...
  const handleDownloadErrorReport = () => {
    ...
    const header = ["Row", "Company Name", "Customer Code", "Error"];
    const lines = [header.map(csvEscape).join(",")];
    for (const fd of summary.failed_details) {
      const originalRow = preview?.rows?.find((r) => r.row_index === fd.row);
      const company = originalRow?.mapped_data?.company_name || originalRow?.raw_data?.company_name || originalRow?.raw_data?.companyName || "";
      const code = originalRow?.mapped_data?.customer_code || originalRow?.raw_data?.customer_code || originalRow?.raw_data?.customerCode || "";
      lines.push([fd.row, company, code, fd.error].map(csvEscape).join(","));
    }
    const blob = new Blob([lines.join("\n")], { type: "text/csv;charset=utf-8;" });
    triggerDownload(blob, `customer-import-error-report-${new Date().toISOString().slice(0, 10)}.csv`);
  };

  const handleDownloadTemplate = async (fmt) => {
    setTemplateLoading(true);
    try {
      await customerApi.downloadTemplate(fmt);
    } catch (e) {
  ...
  const handlePreview = async (advance = true) => {
    ...
    try {
      ...
      const result = await customerApi.importPreview(fd);
      setPreview(result);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      while (!isComplete) {
        const batch = await customerApi.importConfirm({
      ...
      <div className="space-y-4">
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-start gap-2">
          ...
          <p className="text-xs text-blue-700">
            Map each column from your file to the corresponding customer field.
  ...
  const renderStep3 = () => (
    <div className="flex flex-col items-center justify-center py-16 gap-4">
      ...
      <p className="text-sm text-slate-500 text-center max-w-xs">
        Checking required fields, validating values and detecting duplicates.
      </p>
    </div>
  );
    ...
    if (confirmProgress) {
      ...
      return (
        <div className="flex flex-col items-center justify-center py-16 gap-4">
          ...
          <p className="text-sm text-slate-500 text-center max-w-xs">
            Processing your customers in batches of {CONFIRM_BATCH_SIZE.toLocaleString()} — please keep this window open.
          </p>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: "Total Rows",  value: total,     color: "bg-slate-100 text-slate-700", icon: Users },
                ...
                {visibleRows?.map((row) => (
                  <tr key={row.row_index} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2 font-mono text-slate-600">
                      {row.mapped_data?.customer_code || row.raw_data?.customer_code || row.raw_data?.customerCode || "—"}
                    </td>
      ...
      <div className="flex flex-col items-center text-center py-6 gap-5">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            {success ? `${summary.imported} customer${summary.imported !== 1 ? "s" : ""} imported successfully.` : "No new records were created."}
          </p>
        </div>
  ...
  const nextLabel = () => {
    ...
    if (step === 4) return loading ? "Importing…" : `Import ${(preview?.valid || 0) + (preview?.duplicate || 0)} Customers`;
      ...
      >
        ...
        <div className="p-6 pb-0 bg-gradient-to-r from-brand to-brand-hover text-white flex items-center justify-between rounded-t-3xl">
          <div>
            ...
            <p className="text-brand-100 text-sm mt-0.5">Upload CSV or XLSX to bulk-import your customer list</p>
          </div>
