                    ...
                    <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/billing/invoices/${inv.id}`)}>
                      ...
                      <td className="py-3 px-3 text-center">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          inv.status === 'sent' || inv.status === 'partially_paid' ? 'bg-amber-100 text-amber-700' :
                          inv.status === 'overdue' ? 'bg-red-100 text-red-700' :
                          inv.status === 'cancelled' || inv.status === 'written_off' ? 'bg-slate-100 text-slate-600' :
                          inv.status === 'refunded' ? 'bg-pink-100 text-pink-700' : 'bg-gray-100 text-gray-600'
