"""
migrations/add_commercial_account_stripe_customer_id_column.py
------------------------------------------------------------------
Idempotent, additive-only migration that adds the
`commercial_accounts.stripe_customer_id` column on EXISTING databases
...
Usage:
    python -m migrations.add_commercial_account_stripe_customer_id_column            # apply
    python -m migrations.add_commercial_account_stripe_customer_id_column --check    # report only
...
(once approved) from backend/ with BILLING_DATABASE_URL set:
    ...
    python -m migrations.add_commercial_account_stripe_customer_id_column
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add commercial_accounts.stripe_customer_id (idempotent, additive).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    with engine.begin() as conn:
        conn.execute(text(f"ALTER TABLE {TABLE_NAME} ADD COLUMN {COLUMN_NAME} VARCHAR(255)"))
