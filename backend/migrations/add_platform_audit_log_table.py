"""
migrations/add_platform_audit_log_table.py
------------------------------------------
Idempotent, additive-only migration that creates the `platform_audit_logs`
...
registers app.modules.super_admin.models — PlatformAuditLog). This script
...
(app/modules/super_admin/models.py).
...
Usage:
    python -m migrations.add_platform_audit_log_table            # apply
    python -m migrations.add_platform_audit_log_table --check    # report only
...
(once approved) from backend/ with BILLING_DATABASE_URL set:
    ...
    python -m migrations.add_platform_audit_log_table
"""
...
from app.modules.super_admin.models import PlatformAuditLog  # noqa: F401  (registers table)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add the platform_audit_logs table (idempotent, additive).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
