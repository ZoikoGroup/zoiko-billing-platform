"""
migrations/add_commercial_subscription_converted_at_column.py
------------------------------------------------------------------
Idempotent, additive-only migration that adds the
`converted_at` column onto the EXISTING
...
Safe to run any number of times:
  - only adds the column if it's missing (inspector-driven)
  - never drops anything, never deletes data, never recreates tables
  - nullable: NULL means no trial->paid conversion has occurred yet

Usage:
    python -m migrations.add_commercial_subscription_converted_at_column            # apply
    python -m migrations.add_commercial_subscription_converted_at_column --check    # report only
...
(once approved) from backend/ with BILLING_DATABASE_URL set:
    ...
    python -m migrations.add_commercial_subscription_converted_at_column
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add the missing commercial_subscriptions.converted_at column (idempotent, additive).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
