"""
migrations/add_commercial_plan_version_is_placeholder_pricing_column.py
--------------------------------------------------------------------------
Idempotent, additive-only migration that adds the
`commercial_plan_versions.is_placeholder_pricing` column on EXISTING
databases (PostgreSQL or the dev SQLite fallback).
...
Usage:
    python -m migrations.add_commercial_plan_version_is_placeholder_pricing_column            # apply
    python -m migrations.add_commercial_plan_version_is_placeholder_pricing_column --check    # report only
...
(once approved) from backend/ with BILLING_DATABASE_URL set:
    ...
    python -m migrations.add_commercial_plan_version_is_placeholder_pricing_column
"""
...
COLUMN_NAME = "is_placeholder_pricing"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add commercial_plan_versions.is_placeholder_pricing (idempotent, additive).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
