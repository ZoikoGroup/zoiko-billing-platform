"""
migrations/add_commercial_subscription_trial_ends_at_column.py
------------------------------------------------------------------
Idempotent, additive-only migration that adds the
`commercial_subscriptions.trial_ends_at` column on EXISTING databases
...
Usage:
    python -m migrations.add_commercial_subscription_trial_ends_at_column            # apply
    python -m migrations.add_commercial_subscription_trial_ends_at_column --check    # report only
...
(once approved) from backend/ with BILLING_DATABASE_URL set:
    ...
    python -m migrations.add_commercial_subscription_trial_ends_at_column
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add commercial_subscriptions.trial_ends_at (idempotent, additive).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
