"""
migrations/add_billing_kill_switch_table.py
----------------------------------------------
Idempotent, additive-only migration that creates the `billing_kill_switches`
...
registers app.modules.super_admin.models — BillingKillSwitch). This script
...
Usage:
    python -m migrations.add_billing_kill_switch_table            # apply
    python -m migrations.add_billing_kill_switch_table --check    # report only
...
(once approved) from backend/ with BILLING_DATABASE_URL set:
    ...
    python -m migrations.add_billing_kill_switch_table
"""
...
from app.modules.super_admin.models import BillingKillSwitch  # noqa: F401  (registers table)

TABLE_NAME = "billing_kill_switches"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add the billing_kill_switches table (idempotent, additive).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
