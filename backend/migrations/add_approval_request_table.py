"""
migrations/add_approval_request_table.py
------------------------------------------
Idempotent, additive-only migration that creates the `approval_requests`
...
registers app.modules.super_admin.models — ApprovalRequest). This script
exists only because create_all does NOT alter existing databases — see
migrations/create_all/README.md ("Schema changes").

Run this BEFORE migrations/add_commercial_plan_version_table.py —
commercial_plan_versions has a nullable FK to approval_requests.id.
...
Usage:
    python -m migrations.add_approval_request_table            # apply
    python -m migrations.add_approval_request_table --check    # report only
...
(once approved) from backend/ with BILLING_DATABASE_URL set:
    ...
    python -m migrations.add_approval_request_table
"""
...
from app.modules.super_admin.models import ApprovalRequest  # noqa: F401  (registers table)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add the approval_requests table (idempotent, additive).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
