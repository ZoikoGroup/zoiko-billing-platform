"""
migrations/add_commercial_plan_is_quote_only_column.py
--------------------------------------------------------
Idempotent, additive-only migration that adds the
`commercial_plans.is_quote_only` column on EXISTING databases (PostgreSQL or
...
Usage:
    python -m migrations.add_commercial_plan_is_quote_only_column            # apply
    python -m migrations.add_commercial_plan_is_quote_only_column --check    # report only
...
(once approved) from backend/ with BILLING_DATABASE_URL set:
    ...
    python -m migrations.add_commercial_plan_is_quote_only_column
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add commercial_plans.is_quote_only (idempotent, additive).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
