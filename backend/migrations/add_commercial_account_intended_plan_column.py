"""
migrations/add_commercial_account_intended_plan_column.py
-----------------------------------------------------------
Idempotent, additive-only migration that adds the
`commercial_accounts.intended_plan_code` column on EXISTING databases
...
Usage:
    python -m migrations.add_commercial_account_intended_plan_column            # apply
    python -m migrations.add_commercial_account_intended_plan_column --check    # report only
...
(once approved) from backend/ with BILLING_DATABASE_URL set:
    ...
    python -m migrations.add_commercial_account_intended_plan_column
"""
...
COLUMN_NAME = "intended_plan_code"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add commercial_accounts.intended_plan_code (idempotent, additive).",
    )
    parser.add_argument(
        "--check",
        action="store_true",
