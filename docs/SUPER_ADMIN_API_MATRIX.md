...
| GET | `/commercial-accounts` | `listCommercialAccounts` (`AuditLogsPage.jsx`, `Plane1BillingPage.jsx`, `SubscriptionsPage.jsx`) | super_admin | `commercial_accounts`, `organizations` | `CommercialAccountListResponse` | 401/403 | Covered |
...
| GET | `/commercial-plans` | `listCommercialPlans` (`PlansPage.jsx`, `EntitlementsPage.jsx`, `SubscriptionsPage.jsx`, `Plane1BillingPage.jsx`, `PlanEntitlementsPage.jsx`, `EvaluationProgramsPage.jsx`) | super_admin | `commercial_plans` | `CommercialPlanListResponse` | 401/403 | Covered |
...
| GET | `/commercial-subscriptions` | `listCommercialSubscriptions` (`SubscriptionsPage.jsx`) | super_admin | `commercial_subscriptions` | `CommercialSubscriptionListResponse` | 401/403 | Covered |
...
| POST | `/commercial-billing/invoices` | `createPlatformInvoice` (`commandCenterService.js`) | `commercial_financial.write` | `platform_invoices` | `PlatformInvoice` | 400/401/403 | Covered |
| POST | `/commercial-billing/invoices/{id}/finalize` | `finalizePlatformInvoice` (`commandCenterService.js`) | `commercial_financial.write` | `platform_invoices`, `platform_invoice_number_sequences` | `PlatformInvoice` | 400 empty items, 401/403/404 | Covered — atomic numbering |
| POST | `/commercial-billing/invoices/{id}/void` | `voidPlatformInvoice` (`commandCenterService.js`) | `commercial_financial.write` | `platform_invoices` | `PlatformInvoice` | 400 wrong status, 401/403/404 | Covered |
| POST | `/commercial-billing/invoices/{id}/items` | `addPlatformInvoiceItem` (`commandCenterService.js`) | `commercial_financial.write` | `platform_invoice_items` | `PlatformInvoiceItem` | 400 not-DRAFT, 401/403/404 | Covered |
...
| POST | `/commercial-billing/invoices` | `createPlatformInvoice` (`commandCenterService.js`) | `commercial_financial.write` | `platform_invoices`, `commercial_accounts` | `PlatformInvoice` | 400 validation, 401/403 | Covered |
...
| POST | `/commercial-billing/invoices/{id}/finalize` | `finalizePlatformInvoice` (`commandCenterService.js`) | `commercial_financial.write` | `platform_invoices`, `platform_invoice_items`, `platform_invoice_number_sequences` | `PlatformInvoice` | 400 empty items, circuit breaker blocked, 401/403/404 | Covered — atomic numbering via SELECT FOR UPDATE |
| POST | `/commercial-billing/invoices/{id}/void` | `voidPlatformInvoice` (`commandCenterService.js`) | `commercial_financial.write` | `platform_invoices` | `PlatformInvoice` | 400 wrong status, 401/403/404 | Covered |
| POST | `/commercial-billing/invoices/{id}/items` | `addPlatformInvoiceItem` (`commandCenterService.js`) | `commercial_financial.write` | `platform_invoice_items` | `PlatformInvoiceItem` | 400 not-DRAFT, 401/403/404 | Covered |
...
## `/api/super-admin/organizations/*` (directory + governed lifecycle — the actual UI path for org listing & state changes)

| Method | Endpoint | Frontend caller | Role | DB dependency | Expected response | Error behavior | Test status |
|---|---|---|---|---|---|---|---|
| GET | `/organizations` | `listOrganizations` (`OrganizationsPage.jsx`, `OverridesPage.jsx`, `UsageDiagnosticsPage.jsx`) | super_admin | `organizations` (identity + lifecycle + operational counts) | `OrganizationDirectoryResponse` | 401/403 | Covered |
| GET | `/organizations/{organizationId}/overview` | `getOrganizationOverview` | super_admin | `organizations` | `OrganizationOverviewResponse` | 401/403/404 | Covered |
| POST | `/organizations/{organizationId}/lifecycle-transition` | `transitionOrganizationLifecycle` (`OrganizationDetailPage.jsx`) | super_admin | `organizations`, `platform_audit_logs` | `LifecycleTransitionResponse` | 400 invalid target / illegal transition, 404 unknown org, 401/403; audited | Covered — governed lifecycle state machine |
...
| PATCH | `/{id}/status` | `setOrganizationStatus` (**declared in `commercialService.js`, no current call site** — the UI changes org state via `transitionOrganizationLifecycle` → `POST /api/super-admin/organizations/{id}/lifecycle-transition`) | super_admin | `organizations`, `platform_audit_logs` | `OrganizationResponse` | 401/403/404; audited (ACTIVATE/DEACTIVATE) | Covered |
...
| DELETE | `/{id}` | *(rejected — no `deleteOrganization` frontend function exists anywhere; the page uses the governed lifecycle endpoint instead)* | super_admin | `organizations` | `SuccessResponse` (declared) — but the endpoint ALWAYS raises `BadRequestException` (400): organizations cannot be hard-deleted because billing history must be retained; the supported path is `POST /api/super-admin/organizations/{id}/lifecycle-transition` | 400 always (even for an existing org), 404 unknown org, 401/403 | Covered |

## Dead endpoints / no frontend consumer

- **`GET /api/organizations/` (list_organizations, super_admin only)** — no frontend caller found. `OrganizationsPage.jsx` lists organizations via `listOrganizations` → `GET /api/super-admin/organizations` (an identity + lifecycle + operational-count directory view) instead, so this plain organization list endpoint is currently orphaned. Not a bug (it still works and is tested at the service level), just unused — worth a product decision on whether to remove it or wire a "simple list" view to it.
...
- **`PATCH /api/organizations/{id}/status`** via `setOrganizationStatus` (commercialService.js) — the service function exists but no page calls it. `OrganizationDetailPage.jsx` changes an organization's state through `transitionOrganizationLifecycle` (`POST /api/super-admin/organizations/{id}/lifecycle-transition`) instead, which enforces the governed lifecycle state machine. The legacy status-toggle endpoint and its service wrapper are orphaned — flagged for a future cleanup decision (the endpoint itself remains audited and tested).
- **`DELETE /api/organizations/{id}`** — no `deleteOrganization` frontend function exists and the backend always rejects with 400; hard-delete is intentionally unsupported in favor of the governed lifecycle offboarding path.
