...
| `SETUP_KEY` | Required to run `scripts/seed_super_admin.py`. Never exposed via any HTTP endpoint. |
| `STRIPE_SECRET_KEY` / `STRIPE_PUBLISHABLE_KEY` / `STRIPE_WEBHOOK_SECRET` / `STRIPE_CURRENCY_DEFAULT` | Stripe Checkout + webhooks (`stripe_service.py`, `stripe_router.py`, `webhook_router.py`). Inert until set — nothing requires them to boot. |
